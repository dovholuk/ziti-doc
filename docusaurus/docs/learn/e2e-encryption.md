...
traditional network, security is often fragmented across different layers. Ziti focuses primarily on **cryptographic
security**, which serves as a powerful mitigant for weaknesses in other areas, such as physical or transmission
security.
...
- **End-to-Edge Encryption**: Data is encrypted within the Ziti SDK at the source and remains encrypted throughout the
  entire Ziti Fabric. It's only decrypted by the destination SDK, meaning even a compromised Ziti router can't "see" the
  raw data.

## Technical requirements and implementation

Ziti's encryption implementation is designed to be lightweight, avoiding additional network round trips while
maintaining industry-standard security.

### The libsodium library

Rather than implementing custom cryptography, NetFoundry utilizes libsodium, a widely audited, open-source library used
by major platforms like Discord and WireGuard. This provides cross-platform support across all Ziti SDK flavors,
including Go, C, Java, and Android.

### Encryption algorithms

Ziti uses the **XChaCha20-Poly1305** cipher.

- **Performance**: ChaCha20 is significantly faster than AES-256 on devices without hardware-accelerated AES, such as
  many mobile phones and IoT edge devices.
- **Strength**: It provides 256-bit security, which is equivalent to the strength of AES-256.

## How the Ziti key exchange works

Ziti leverages established trust via an ephemeral key exchange to bootstrap a secure session without adding latency.
...
For every message sent, Ziti performs the following:

- **Encryption & Authentication**: Data is encrypted and a Message Authentication Code (MAC) is appended to ensure
  integrity.
- **Key Rotation**: The transmission (tx) and reception (rx) keys are rotated immediately after every message. This
  provides **Perfect Forward Secrecy**, ensuring that if a future key is somehow compromised, past sessions remain
  secure.
...
Edge-to-edge encryption is **enabled by default** for all SDK-to-SDK communications. However, it can be made optional
via service configuration. This is useful for services that already use high-end encryption (like proxied HTTPS),
allowing you to avoid "double-encrypting" data where performance is the absolute priority.
